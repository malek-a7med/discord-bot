...
import ClaimStore from "./helpers/claim-store.js";

// ── Lock File (الإصدار الجديد): heartbeat-based single instance ────
// بنستبدل الـ lock القديم (اللي كان بيعتمد على process.kill) بنظام
// heartbeat بيشتغل صح على Replit. الـ acquireInstanceLock() بترجع
// { acquired, ... } — لو false بنخرج فوراً.
import { acquireInstanceLock, getLockStatus } from "./helpers/single-instance.js";
const _lockResult = acquireInstanceLock();
if (!_lockResult.acquired) {
  console.error(`❌ [Lock] ${_lockResult.reason} — النسخة التانية: PID=${_lockResult.existingPid}, آخر heartbeat من ${Math.floor(_lockResult.existingHeartbeatAge / 1000)}s. بخلي النسخة دي تخرج.`);
  setTimeout(() => process.exit(0), 200);
  throw new Error("Bailing out: another instance is already running.");
}
console.log(`🔒 [Lock] احنا النسخة الشرعية (instanceId=${_lockResult.instanceId}${_lockResult.fresh ? "" : `, tookOverFrom=${_lockResult.tookOverFrom}`})`);

// ── ClaimStore: منع تكرار الردود عبر instances مختلفة بـ atomic writes ───
// بنستخدمه بدلاً من الـ in-memory Set + read-modify-write في database.js
// بنعمل initialize مع نفس الـ instanceId بتاع الـ lock
const _claimStore = new ClaimStore({
  filePath: path.join(__dirname, "data", "claims-store.json"),
  ttlMs: 10 * 60 * 1000,   // 10 دقايق
  instanceId: _lockResult.instanceId,
});
console.log(`🗂️  [ClaimStore] مهيأ — مسار الملف: ${path.join(__dirname, "data", "claims-store.json")}`);
...
client.on("messageCreate", async (msg) => {
  ...
  // بنستخدم ClaimStore (atomic write على الـ disk) مش بس in-memory Set.
  // كده لو في نسختين شغالين، واحدة بس هتكممل والباقي بيخرج.
  // الـ in-memory Set بيفضل كـ fast path للحالة العادية (نفس الـ instance).
  if (processedMessages.has(msg.id)) return;
  // claim على مستوى الـ disk (بيشتغل بين instances)
  if (!_claimStore.tryClaim(`msg:${msg.id}`)) {
    // نسخة تانية بتعامل مع نفس الرسالة — بنخرج
    return;
  }
  ...
  if (msg.guild) {
    ...
    if (amResult?.triggered) {
      ...
      if (amResult.logData) {
        ...
        // Claim على الـ disk عشان لو في نسختين بيشوفوا نفس الرسالة
        // واحدة بس هتعمل اللوج. الـ key مبني على userId+content+timestamp
        // عشان لو الرسالة اتكررت بـ id مختلف نلاقيها.
        const logKey = `automod:${ld.userId}:${ld.channelId}:${ld.savedContent?.slice(0, 50) ?? ""}:${Math.floor((ld.timestamp ?? 0) / 1000)}`;
        if (!_claimStore.tryClaim(logKey)) return;
    ...
    if (/^(السلام\s*عليكم|سلام\s*عليكم)/i.test(msg.content.trim())) {
      ...
      // Claim على الـ disk أولاً (حماية cross-instance)
      const salaamKey = `salaam:${msg.channel.id}:${Math.floor(Date.now() / 60_000)}`;
      if (!_claimStore.tryClaim(salaamKey)) return;
  ...
  if (isOwner) {
    if (!_claimStore.tryClaim(`ai-owner:${msg.id}`)) return;
  ...
  // ClaimStore بيكتب atomic على الـ disk — آمن بين instances مختلفة
  if (!_claimStore.tryClaim(`ai:${msg.id}`)) return;
...
client.on("interactionCreate", async (interaction) => {
  ...
  // بنستخدم ClaimStore (atomic على الـ disk) + in-memory Set كـ fast path.
  if (processedInteractions.has(interaction.id)) return;
  if (!_claimStore.tryClaim(`interaction:${interaction.id}`)) return;
...
client.on('guildMemberAdd', async (member) => {
  // Claim على مستوى الـ disk أولاً — لو نسخة تانية بتتعامل مع نفس العضو، نخرج
  if (!_claimStore.tryClaim(`welcome:${member.guild.id}:${member.id}`)) return;
...
client.on('guildMemberRemove', async (member) => {
  // Claim على مستوى الـ disk أولاً
  if (!_claimStore.tryClaim(`leave:${member.guild.id}:${member.id}`)) return;
...
app.get('/status', (_req, res) => {
  ...
  res.json({
    ...
    instance: {
      pid: lock.currentPid,
      instanceId: lock.currentInstanceId,
      isPrimary: lock.isOwner,
      existingLock: lock.existing,
    },
    claims: {
      cached: _claimStore.size(),
    },
  });
});

...
function buildSuggestionsPanelEmbed() {
  return new EmbedBuilder()
    ...
    .setDescription(
      ...
      "🖼️ **إضافة صورة لاقتراحك؟**\n" +
      "بعد ما تبعت اقتراحك عن طريق الأزرار، ابعت الصورة هنا في الروم ده مباشرةً وهتتضاف لاقتراحك تلقائياً."
    )
    .setFooter({ text: '⚜️ سيرفر الفراعنة ✨ | نظام التطوير المستمر' })
    .setTimestamp();
}
...
async function ensureSuggestionsPanel(clientInstance, force = false) {
  try {
    ...
    if (!force && existingPanels.size > 0) {
      // ✅ عدّل الرسالة الموجودة بدل ما نتجاهلها — عشان أي تعديل في النص يظهر
      const existingPanel = existingPanels.first();
      await existingPanel.edit({
        embeds: [buildSuggestionsPanelEmbed()],
        components: [buildSuggestionsPanelRow()],
      }).catch(() => {});
      logger.info("ℹ️ لوحة الاقتراحات الثابتة موجودة بالفعل — تم تحديثها");
      return true;
    }

...
client.on("messageCreate", async (msg) => {
  ...
  if (!msg.guild) {
    ...
    try {
      ...
      console.error("[DM] خطأ:", err.message);
      if (err.isQuotaError || err.message === "ALL_KEYS_EXHAUSTED") {
        return msg.channel.send("⏳ الـ AI وصل الحد اليومي — جرب بعد شوية!").catch(() => {});
      }
        ...
        try {
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              ...
              `📡 **السيرفر:** ${msg.guild?.name ?? "غير معروف"}\n\n` +
              `هتطرده ولا تسيبه؟`
            )
            .setTimestamp();
  ...
  if (interaction.isChatInputCommand()) {
    ...
    try {
      ...
      if (cmd === "قائمة-مبلوكين") {
        ...
        if (blocked.length === 0) {
          return interaction.reply({ content: "✅ مفيش حد مبلوك دلوقتي.", flags: 64 });
        }
        return interaction.reply({
      ...
      if (cmd === "رفع-بلوك") {
        ...
        if (!entry || entry.blockedUntil <= Date.now()) {
          return interaction.reply({ content: `✅ **${target.username}** مش مبلوك أصلاً.`, flags: 64 });
        }
        entry.blockedUntil = 0;
        entry.timestamps   = [];
        return interaction.reply({ content: `✅ اترفع البلوك عن **${target.username}** بنجاح.`, flags: 64 });
      }
      ...
      if (cmd === "serverinfo") {
        if (!guild) return interaction.reply({ content: "❌ الأمر ده في السيرفرات بس يسطا!", flags: 64 });
        const embed = new EmbedBuilder()
          .setColor(0x7289da)
      ...
      if (cmd === "شراء") {
        ...
        if (uData.coins < prices[choice]) return interaction.reply({ content: "❌ معندكش كوينز كفاية يا غالي!", flags: 64 });

        const role = guild.roles.cache.find(r => r.name.toLowerCase() === choice);
        if (!role) return interaction.reply({ content: "❌ الرتبة مش متأسسة بنفس الاسم في السيرفر ده!", flags: 64 });
      ...
      if (cmd === "يومي") {
        const uData = db.getUser(user.id);
        const now = Date.now();
        if (uData.lastDaily && now - uData.lastDaily < 86400000) return interaction.reply({ content: "❌ أخدت المكافأة بتاعتك النهاردة خلاص يسطا تعالي بكره!", flags: 64 });
      ...
      if (cmd === "مانهوا-إنشاء") {
        ...
        if (db.getManhwaDict(name) && Object.keys(db.getManhwaDict(name)).length > 0) return interaction.reply({ content: "❌ القاموس ده موجود بالفعل يسطا!", flags: 64 });
      ...
      if (cmd === "مانهوا-إضافة-مصطلح") {
        ...
        if (!db.getManhwaDict(mName) || Object.keys(db.getManhwaDict(mName)).length === 0) return interaction.reply({ content: "❌ القاموس ده مش موجود، اعمله الأول!", flags: 64 });
      ...
      if (cmd === "مانهوا-عرض-المصطلحات") {
        ...
        if (mName) {
          ...
          if (!dict || Object.keys(dict).length === 0) return interaction.reply({ content: "❌ مش موجود القاموس ده!", flags: 64 });
      ...
      if (cmd === "تشغيل-اختبار") {
        ...
        if (!testChannel) return interaction.reply({ content: "❌ مش لاقي قناة الترحيب!", flags: 64 });
    ...
    try {
      ...
      if (interaction.customId.startsWith("dmp_")) {
        if (!config.isOwner(interaction.user.id)) {
          return interaction.reply({ content: "❌ اللوحة دي للأونر بس!", flags: 64 });
        }
      ...
      if (interaction.customId.startsWith("automod_kick_yes_") || interaction.customId.startsWith("automod_kick_no_")) {
        if (!config.isOwner(interaction.user.id)) {
          return interaction.reply({ content: "❌ القرار ده للأونر بس!", flags: 64 });
        }
        ...
        if (!isKick) {
          return interaction.followUp({ content: "✅ تمام، سيبناه المرة دي. لو كررها هيجيلك تقرير تاني.", flags: 64 });
        }

        if (!guild) return interaction.followUp({ content: "❌ مش لاقي السيرفر!", flags: 64 });

        try {
          await guild.members.fetch().catch(() => {});
          const member = guild.members.cache.get(targetId);
          if (!member) return interaction.followUp({ content: "❌ العضو مش موجود أو طلع بنفسه!", flags: 64 });
          ...
          return interaction.followUp({ content: `❌ فشلت في الطرد: ${err.message}`, flags: 64 });
        }
      }
      // ─────────────────────────────────────────────────────────────

...
- **تحديث (يوليو 2026):** المستخدم وضّح إنه أصلاً معتمد على بوتات تانية لأي لوج، فمش عايز أي لوج من هذا البوت نهائي (غير التذاكر اللي متستبعدة). اتقفلوا كمان: `sendModLog` (لوج أفعال المشرفين اليدوية زي /كتم /باند)، `notifyOwnersOfError` (تنبيه خطأ الأوامر للأونر — قناة و DM)، لوج الأوتومود AI في `AUTO_MOD_LOG_CHANNEL_ID` (كان فيه أزرار تصرف سريع aml_restore/aml_confirm/aml_mute/aml_kick/aml_ban مبنية على اللوج ده — دلوقتي متقفولة برضو)، و `Logger.sendToAdmin` في logger.js (كانت بتغذي logModerationAction/logAPIError). نظام الحماية (Anti-Raid/Anti-Nuke) نفسه (مش بس لوجه) مقفول بمفتاح `PROTECTION_ENABLED` في index.js.
- **Why:** طلب صريح ومتكرر من المستخدم "اقفل أي لوج تبع البوت كله حرفيا" + توضيح إنه معتمد على بوتات تانية لكل اللوجات.
- **How to apply:** لو المستخدم طلب رجّع لوج معيّن، رجّع الدالة المحددة بس (مش كلهم)، وفكر لو رجّع نظام الحماية إن أزرار الـ automod log القديمة (aml_*) هترجع تشتغل تلقائي.
